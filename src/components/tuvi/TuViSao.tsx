...
type DacTinhType = 'Vượng' | 'Miếu' | 'Đắc' | 'Bình' | 'Hãm' | '';
...
const TuViSao = ({ sao, compact = false }: TuViSaoProps) => {
  ...
  const dacTinh = dacTinhStyles[sao.dacTinh as DacTinhType] || dacTinhStyles[''];
  const tuHoa = sao.tuHoa ? tuHoaStyles[sao.tuHoa] : null;

  return (
    <span className={cn('inline-flex items-center gap-0.5 leading-tight', nguHanhColors[sao.nguHanh], isChinhTinh ? 'font-medium' : 'font-normal', compact ? 'text-[9px]' : 'text-[10px]')}>
      <span className={isChinhTinh ? 'underline decoration-dotted underline-offset-2' : ''}>{sao.ten}</span>
      {dacTinh.text && <sup className={cn('text-[7px] -top-0.5', dacTinh.color)}>{dacTinh.text}</sup>}
      {tuHoa && <sup className={cn('text-[7px] -top-0.5 ml-0.5', tuHoa.color)}>{tuHoa.abbr}</sup>}
    </span>
  );
};
